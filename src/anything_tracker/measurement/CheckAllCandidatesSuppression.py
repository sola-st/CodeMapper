...
class MeasureSuppression():
    ...
    def run(self):
        ...
        for repo in repo_folders:
            ...
            # predicted
            candidates_folder = join(self.results_dir, repo)
            folders = os.listdir(candidates_folder)
            for folder in folders:
                current_sub_folder = join(candidates_folder, folder)
                if os.path.isdir(current_sub_folder):
                    json_results_file = join(current_sub_folder, f"candidates.json")
                    ...
                    all_candidates_num = len(histories_regions_all)
                    for i, region in enumerate(histories_regions_all):
                        ground_truth_idx = folder
                        ...
                        if region["kind"] == "no candidate regions" and "off_diff" in self.results_csv_file:
                            ...
                            self.candidate_nums.append(all_candidates_num)
                            self.target_region_indices.append(i)
