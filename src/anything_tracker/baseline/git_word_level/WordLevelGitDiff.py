...
from anything_tracker.baseline.git_word_level.GitWordToCandidateRegion import WordGitToCandidateRegion
from anything_tracker.utils.ReadFile import get_region_characters
from anything_tracker.baseline.git_word_level.FineGrainWordIndices import FineGrainWordIndices


class WordLevelGitDiff():
    ...
    def run_git_diff(self):
        ...
        commit_diff_command = f"git diff --color --unified=0 --word-diff -w {self.base_commit} {self.target_commit} -- {self.file_path}"
        ...
        if not candidate_regions:
            candidate_regions = WordGitToCandidateRegion(self, top_diff_hunks, middle_diff_hunks, bottom_diff_hunks).get_diff_maps()
                ...
                if overlapped_line_numbers: # range overlap
                    if self.interest_first_number in overlapped_line_numbers and candidate_character_start_idx_done == False:
                        if self.characters_start_idx == 1:
                            ...
                            candidate_character_start_idx = fine_grain_start.fine_grained_word_indices()
                    ...
                    if self.interest_last_number in overlapped_line_numbers and candidate_character_end_idx_done == False:
                        ...
                        candidate_character_end_idx = fine_grain_end.fine_grained_word_indices()
                    ...
                    if list(set(self.interest_line_numbers) - set(base_hunk_range_list)) == []: 
                        if candidate_character_end_idx == 0 or candidate_character_end_idx == None:
