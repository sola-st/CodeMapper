...
class WordGitToCandidateRegion():
    ...
    def get_diff_maps(self):
        ...
        if self.top_diff_hunk:
            ...
            start_char = self.middle_diff_hunks[0].target_start_character
            ...
            region_range = [start_line, start_char, end_line, end_char]
    ...
    def combine_diff_and_unchanged_ranges(self, location):
        ...
        if location == "top":
            ...
            if not self.bottom_diff_hunk:
                ...
                candidate_characters = self.target_file_lines[end_line-1]
                end_char = len(candidate_characters)
                region_range = [self.top_diff_hunk.base_start_line_number, self.top_diff_hunk.target_start_character, end_line, end_char]
                candidate_region_range = CharacterRange(region_range)
                marker = "<TOP_OVERLAP>"
            ...
            start_char = specified_diff_hunks[0].target_start_character
            end_line = self.bottom_diff_hunk.target_end_line_number - 1
            candidate_characters = self.target_file_lines[end_line-1]
            end_char = len(candidate_characters)
            region_range = [start_line, start_char, end_line, end_char]
