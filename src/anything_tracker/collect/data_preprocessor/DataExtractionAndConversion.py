...
class DataExtractionAndConversion():
    ...
    def convert_data(self, file_number, json_file, category, subfolder):
        ...
        converted_json_str_expect, expect_simple = \
        ...
        if converted_json_str_expect: # equal to if expect_simple, they are always in the same status: exists or not.
            ...
            write_extracted_json_strings(file2_for_expect, expect_simple, "w")
    ...
    def analyze_histories(self, repo_dir, change_histories, category, repo_url, file_number):
        ...
        for h in change_histories:
            ...
            elif source_range == "drop_current_data":
                continue # multi ranges found, filter out current history
            else:
            ...
            expect_simple.update(extracted_pieces_1)
            extracted_pieces_2 = get_commit_range_pieces(target_commit, target_file, target_range_to_json)
            expect_simple.update(extracted_pieces_2)
        
        return extracted_change_histories, expect_simple
    ...
    def analyze_histories_group_2(self, repo_dir, h, source_reference, target_reference, category, file_number):
        ...
        child_commit = h["commitId"]
        repo.git.checkout(child_commit, force=True)
        target_file_path = join(repo_dir, h["elementFileAfter"])  
