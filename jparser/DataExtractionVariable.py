...
def get_meta_info(change_pair):
    ...
    element_name = postfix.split(":", 1)[0]
    element_line = int(postfix.rsplit("(", 1)[1].replace(")", ""))
    return commit, file_path, element_name, element_line

def get_variable_location(repo_dir, commit, file_path, variable_name, variable_line):
    detailed_location = None
    repo = Repo(repo_dir)
    repo.git.checkout(commit, force=True)
    if join(repo_dir, file_path): 
        subprocess.run(["javac", "-cp", "javaparser-core-3.24.2.jar", "VariableDeclarationLocator.java"])
        result = subprocess.run([
            "java",
            "-cp", ".:javaparser-core-3.24.2.jar",
            "VariableDeclarationLocator",
...
def main():
    ...
    for file_idx, file in enumerate(element_source_files):
        ...
        for i, change_pair in enumerate(change_list):
            ...
            target_commit, target_file_path, target_element_name, target_element_line = get_meta_info(change_pair)
            
            # newer commit related
            source_commit = None
            source_file_path = None
            source_element_name = None
            source_element_line = None
            if i+1 == max_change:
                source_commit = info["startCommitId"][:8]
                source_file_path = info["filePath"]
                source_element_name = info["variableName"]
                source_element_line = info["variableStartLine"]
            else:
                source_change_pair = change_list[i+1]
                source_commit, source_file_path, source_element_name,  source_element_line = get_meta_info(source_change_pair)
            ...
            # target
            target_location = get_variable_location(repo_dir, target_commit, target_file_path, target_element_name, target_element_line)
            # source
            source_location = get_variable_location(repo_dir, source_commit, source_file_path, source_element_name, source_element_line)
